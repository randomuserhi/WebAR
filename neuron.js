...
}));
{
    ...
    for (const particle of particles) {
        sections.push(new Section(8, 16, (t, ctx, bezier) => {
            ctx.globalAlpha = (t < 0.5 ? Math.clamp01(t / 0.1) : Math.clamp01((1 - t) / 0.1));
            drawImage(ctx, particle.img, particle.start[0] + (particle.vel[0] * bezier(t)), particle.start[1] + (particle.vel[1] * bezier(t)), bezier(t) * Math.deg2rad * particle.velRot + particle.startRot * Math.deg2rad, particle.scale);
            ctx.globalAlpha = 1;
        }, () => {
...
sections.push(new Section(9, 16, (t, ctx) => {
    ...
    drawImage(ctx, cell_wall, 10, 0);
    {
        ...
        drawImage(ctx, particle.img, particle.end[0] + (particle.vel[0] * (1 - t)), particle.end[1] + (particle.vel[1] * (1 - t)), 0, particle.scale);
    }
    {
        const particle = {
            end: [-200, -220],
    ...
    }
    {
        ...
        drawImage(ctx, particle.img, particle.end[0] + (particle.vel[0] * (1 - t)), particle.end[1] + (particle.vel[1] * (1 - t)), (1 - t) * Math.deg2rad * particle.velRot + particle.endRot * Math.deg2rad, particle.scale);
    }
    {
        const particle = {
            end: [100, 150],
